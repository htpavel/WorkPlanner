...
// 🛡️ Middleware pro kontrolu rolí
function authorize(allowedRoles = []) {
    return (req, res, next) => {
        ...
        if (!authHeader) {
            ...
            return res.status(401).json({ error: "Přístup odepřen. Chybí autorizační hlavička." });
        }
        ...
        if (allowedRoles.length > 0 && !allowedRoles.includes(req.user.role)) {
            return res.status(403).json({ error: `Nedostatečná práva. Vyžadována jedna z rolí: ${allowedRoles.join(', ')}` });
        }
...
// Vytvoření rezervace
router.post('/booking', authorize(["DISPATCHER", "CUSTOMER", "GUEST"]), (req, res) => {
    // Pro routeId v body sice máme zjednodušenou validaci, ale pro jistotu:
...
// Smazání rezervace
router.delete('/booking/:id', authorize(["DISPATCHER", "CUSTOMER"]), (req, res) => {
...
router.patch('/:id/config', authorize(["DISPATCHER"]), (req, res) => {
    const validation = RouteValidation.validateConfigUpdate(req.body);
    if (!validation.isValid) return res.status(400).json({ errors: validation.errors });

    try {
        const result = RouteAbl.updateConfig(req.params.id, req.body);
        res.json(result);
    } catch (error) {
        res.status(400).json({ error: error.message });
    }
